...
const REQUIRED_XML_CHECKS: Array<{ key: XmlCheckKey; label: string; requiredFields: string[] }> = [
  {
    key: "demands",
    label: "PEGDMDMST",
    requiredFields: [
...
type XmlCheckKey = "demands" | "jobs" | "links" | "poDetails" | "supplies" | "partDescriptions";
...
function buildFileChecks(files: UploadFile[]): JobFileCheck[] {
  ...
  const checks: JobFileCheck[] = REQUIRED_XML_CHECKS.map((spec) => {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/PriorsPart_logo_webversion_small_400x150.png") {
    try {
      const logoPath = path.resolve(process.cwd(), "PriorsPart_logo_webversion_small_400x150.png");
