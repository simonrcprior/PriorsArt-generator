...
const peggingSchema = z.object({
  ...
  quantity: z.number().nonnegative(),
  nest: z.number().int().min(1),
  nestText: z.string().optional(),
  parentLinkId: z.string().min(1).nullable(),
  parentDemandId: z.string().min(1).nullable(),
  parentSupplyId: z.string().min(1).nullable(),
...
export function validateCanonical(datasets: CanonicalDatasets, diagnostics: DiagnosticCollector): void {
  ...
  const nestLevelCounts = new Map<number, number>();
  ...
  const duplicateLikeGroups = new Map<string, typeof datasets.peggingLinks>();
  ...
  diagnostics.setNestLevelCounts(
    [...nestLevelCounts.entries()]
      .sort((a, b) => a[0] - b[0])
      .reduce<Record<string, number>>((acc, [nest, count]) => {
