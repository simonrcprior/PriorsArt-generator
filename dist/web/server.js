...
async function ensureJobDir(jobId) {
    return promises_1.default.mkdtemp(node_path_1.default.join(node_os_1.default.tmpdir(), `priorsart-${jobId}-`));
}
...
async function runJob(jobId, body) {
    ...
    for (const file of body.files) {
        const targetPath = node_path_1.default.join(tempDir, resolveUploadPath(file.relativePath, file.name));
        const buffer = Buffer.from(file.contentBase64, "base64");
        await promises_1.default.mkdir(node_path_1.default.dirname(targetPath), { recursive: true });
        await promises_1.default.writeFile(targetPath, buffer);
        writtenNames.push(node_path_1.default.basename(file.name));
    }
    ...
    const manifestPath = manifestName ? node_path_1.default.join(tempDir, manifestName) : undefined;
    ...
    try {
        ...
        const pkg = await (0, generate_1.generateFromXml)({
            inputFile: sourcePath,
            outputFile,
            datePolicy: { defaultDateOrder: normalizeDateOrder(body.dateOrder) },
            ...(manifestPath ? { xmlConfigFile: manifestPath } : {}),
...
function normalizeOutputName(rawOutputName, format) {
...
function resolveOutputContentType(filePath) {
...
function hasAdminRights(roleMode, adminKey) {
    void adminKey;
    return roleMode === "admin";
}
function resolveUploadPath(relativePath, fallbackName) {
    const candidate = (relativePath && relativePath.trim() ? relativePath : fallbackName).replace(/\\/g, "/");
    const normalized = node_path_1.default.posix.normalize(candidate).replace(/^\/+/, "");
    const parts = normalized.split("/").filter((segment) => segment && segment !== "." && segment !== "..");
    if (!parts.length) {
        return node_path_1.default.basename(fallbackName);
    }
    if (parts.length === 1) {
        return parts[0] ?? node_path_1.default.basename(fallbackName);
    }
    return parts.slice(1).join("/") || node_path_1.default.basename(fallbackName);
}
const server = node_http_1.default.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname.startsWith("/api/jobs/") && url.pathname.endsWith("/download")) {
        ...
        try {
            const buffer = await promises_1.default.readFile(job.outputFile);
            res.writeHead(200, {
                "Content-Type": resolveOutputContentType(job.outputFile),
                "Content-Disposition": `attachment; filename="${node_path_1.default.basename(job.outputFile)}"`,
